	    ...
	    if (isRelease()) {
	        stage('Release') {
				...
				docker.build(releaseImageName, "-f Dockerfile.release .")
					...
					sh "ls -al && pwd && hostname && docker run --rm -v /home/jenkins/.ssh/id_rsa:/root/.ssh/id_rsa -v \$(pwd):/usr/src/app ${releaseImageName} bash -c 'ls -al && pwd && hostname && ls -al && make release'"
				}
